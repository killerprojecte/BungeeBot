...
class CodeListener: SubscribeListener {
    ...
    fun onGroupChat(event: GroupMessageEvent){
        ...
        if (MapUtil.updatesMap.containsKey(message)){
            ...
            event.group.quoteMessage(PlainText(BungeeBot.instance!!.config!!.getString("group-msg.ip-update")), event.messageSource.messageId.toString())
            ...
            if (SQLData.playerDTOs.queryBuilder().where().eq("qq", event.sender.id).query().size!=0){
                event.group.quoteMessage(PlainText(BungeeBot.instance!!.config!!.getString("group-msg.exist")), event.messageSource.messageId.toString())
                return
            }
            ...
            event.group.quoteMessage(PlainText(BungeeBot.instance!!.config!!.getString("group-msg.success")), event.messageSource.messageId.toString())
        }
    }
}